...
func HandleRequest(req events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	if len(query) > 2 && len(query) < 20 {
		...
		for i, item := range uniqueItems {
			html[i] = fmt.Sprintf("<div><button class=\"text-2xl lg:text-base px-4 py-2 hover:bg-gray-100\" hx-get=\"/v1/search\" name=\"ingredient\" hx-target=\"#existing-searches\" hx-swap=\"beforeend\" value=\"%s\" hx-on:click=\"const search = document.getElementById('ingredient');search.value = '';search.dispatchEvent(new Event('keyup'));\">%s</button></div>", item.Title, item.Title)
		}
