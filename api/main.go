...
func HandleRequest(req events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	if len(query) > 2 && len(query) < 20 {
		...
		for i, item := range uniqueItems {
			html[i] = fmt.Sprintf("<div><button class=\"text-3xl lg:text-base px-4 py-2 hover:bg-gray-100\" hx-get=\"/v1/search\" name=\"ingredient\" hx-target=\"#existing-searches\" hx-swap=\"beforeend\" value=\"%s\" hx-on:click=\"const search = document.getElementById('ingredient');search.value = '';search.dispatchEvent(new Event('keyup'));\">%s</button></div>", item.Title, item.Title)
		}
	...
	if len(upperFind) > 0 && len(upperFind) < 4 {
		...
		html := make([]string, len(uniqueItems))
		for i, item := range uniqueItems {
			html[i] = fmt.Sprintf("<div><a class=\"text-3xl\" href=\"https://recipes.skran.app/%s.html\">%s</a></div>", item.RecipeId, item.RecipeTitle)
		}
		response = strings.Join(html, "\n")
